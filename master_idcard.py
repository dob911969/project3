...
class IdtypeScreen(MDScreen):
    def __init__(self, **kwargs):
        ...
        title_label = MDLabel(text="Master-Customer ID Card",
                              halign="center",
                              valign="center",
                              font_style="H5",
                              bold=True)
        ...
        self.id_input = MDTextField(hint_text="ID", disabled=True, size_hint_x=None, width=200)
        grid_layout.add_widget(self.id_input)
        self.idtype_input = MDTextField(hint_text="ID Type", size_hint_x=None, width=200)
        ...
        add_button = MDRaisedButton(text="Add",on_release=self.save_id)
    ...
    def generate_id(self):
        ...
        self.id_input.text = str((last_id or 0) + 1)
        conn.close()
    ...
    def save_id(self, *args):
        ...
        #id,idtype
        cursor.execute('''
            INSERT INTO id (id,idtype)
            VALUES (?, ?)
        ''', (id,idtype))
        conn.commit()
        conn.close()
        self.show_confirmation_dialog("id saved successfully!")
        self.reset_fields()
        self.load_id_records_to_table()

    def load_id_records_to_table(self):
        """Load records from the database into the table."""
        self.table.row_data = []  # Clear previous data
        conn = sqlite3.connect("radision.db")
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM id ORDER BY id DESC")
    ...
    def on_row_select(self, instance_table, instance_row):
        ...
        for row in self.table.row_data:
            if row[0] == selected_row:
                self.id_input.text=row[0]
                self.idtype_input.text=row[1]
                break
    ...
    def update_record(self, *args):
        ...
        try:
            ...
            cursor.execute('SELECT 1 FROM id WHERE id = ?', (id,))
            if not cursor.fetchone():
                self.show_confirmation_dialog(f"No record found with id ID {id}.")
                conn.close()
                return
    ...
    def delete_record(self, *args):
        ...
        conn = sqlite3.connect("radision.db")
        cursor = conn.cursor()
        cursor.execute('DELETE FROM id WHERE id = ?', (id,))
        conn.commit()
        conn.close()
