...
class TariffScreen(MDScreen):
    def __init__(self, **kwargs):
        ...
        title_label = MDLabel(text="Master-Roomtariff",
                              halign="center",
                              valign="center",
                              font_style="H5",
                              bold=True)
        ...
        self.tariffid_input = MDTextField(hint_text="Tariff ID", disabled=True, size_hint_x=None, width=200)
        grid_layout.add_widget(self.tariffid_input)
        self.roomtype_input = MDTextField(hint_text="Category Type", size_hint_x=None, width=200)
        grid_layout.add_widget(self.roomtype_input)
        self.tariffamount_input = MDTextField(hint_text="Tariff Amount", size_hint_x=None, width=200)
        grid_layout.add_widget(self.tariffamount_input)
        ...
        add_button = MDRaisedButton(text="Add", on_release=self.save_tariff)
        ...
        fetchroomtype_button = MDRaisedButton(text="Fetch Roomcategory", on_release=self.populate_roomcategory)
        btn_layout.add_widget(fetchroomtype_button)
        ...
        self.generate_tariffid()
        self.load_tariff_records_to_table()
    ...
    def generate_tariffid(self):
        """Generate the next class ID."""
        conn = sqlite3.connect("radision.db")
        cursor = conn.cursor()
        cursor.execute("SELECT MAX(tariffid) FROM tariff")
        last_tariffid = cursor.fetchone()[0]
        self.tariffid_input.text = str((last_tariffid or 0) + 1)
        conn.close()
    ...
    def populate_roomcategory(self, instance):
        ...
        try:
            ...
            if not category:
                self.show_confirmation_dialog("No Roomcategory found in the database.")
                return
            ...
            menu_items = [
                {
                    ...
                    "on_release": lambda x=cat[0]: self.roomtype_menu_callback(x),
                }
                for cat in category
            ]

            self.menu = MDDropdownMenu(
                caller=self.roomtype_input,
                items=menu_items,
                width_mult=4,
            )
            self.menu.open()
    ...
    def roomtype_menu_callback(self, selected_type):
        self.roomtype_input.text = selected_type
        self.menu.dismiss()
    ...
    def save_tariff(self, *args):
        tariffid=self.tariffid_input.text.strip()
        roomtype=self.roomtype_input.text.strip()
        tariffamount=self.tariffamount_input.text.strip()
        
         
        #contact_no = self.contact_no_field.text.strip()
        if not all([tariffid,roomtype,tariffamount]):
        ...
        #floorid,roomtype
        cursor.execute('''
            INSERT INTO tariff (tariffid,roomtype,tariffamount)
            VALUES (?, ?,?)
        ''', (tariffid,roomtype,tariffamount))
        conn.commit()
        conn.close()
        self.show_confirmation_dialog("Tariff saved successfully!")
        self.tariffamount_input.text = ""
        self.generate_tariffid()
        self.load_tariff_records_to_table()

    def load_tariff_records_to_table(self):
        """Load records from the database into the table."""
        self.table.row_data = []  # Clear previous data
        conn = sqlite3.connect("radision.db")
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM tariff ORDER BY tariffid DESC")
    ...
    def on_row_select(self, instance_table, instance_row):
        ...
        for row in self.table.row_data:
            if row[0] == selected_row:
                self.tariffid_input.text=row[0]
                self.roomtype_input.text=row[1]
                self.tariffamount_input.text=str(row[2])
                break
    ...
    def update_record(self, *args):
        ...
        try:
            ...
            cursor.execute('SELECT 1 FROM tariff WHERE tariffid = ?', (tariffid,))
            if not cursor.fetchone():
                self.show_confirmation_dialog(f"No record found with Room ID {tariffid}.")
                conn.close()
                return
            ...
            cursor.execute('''
                ...
                tariffamount =?
                WHERE tariffid = ?
            ''', (roomtype , tariffamount , tariffid))
            conn.commit()
    ...
    def delete_record(self, *args):
        ...
        conn = sqlite3.connect("radision.db")
        cursor = conn.cursor()
        cursor.execute('DELETE FROM tariff WHERE tariffid = ?', (tariffid,))
        conn.commit()
        conn.close()
